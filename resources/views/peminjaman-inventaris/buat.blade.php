...
@section('content')
    ...
    <form id="form-peminjaman" action="{{ action('PeminjamanInventarisController@store') }}" method="POST">
        @csrf
        <div class="form-row">
                    ...
                    @foreach($pelanggans as $pelanggan)
                        ...
                        @elseif(auth()->user()->pelanggan)
                            @if(auth()->user()->pelanggan->kode_pelanggan == $pelanggan->kode_pelanggan)
                                <option value="{{ $pelanggan->kode_pelanggan }}" hidden selected>{{ $pelanggan->nama_pelanggan }}
                                </option>
                            @endif
                        @endif
                    @endforeach
        ...
        <div class="form-row">
            ...
            <div class="form-group ml-5">
                ...
                <input type="text" class="form-control datepicker" id="tanggal" name="tanggal" data-provide="datepicker"
                    required readonly>
        ...
        <table id="table-peminjaman" class="table">
            ...
            <tbody>
                <tr>
                    <td style="width:80%;">
                        <select class="inventaris select2 col-sm-12" name="inventaris[]" id="inventaris_1"
                    ...
                    <input type="hidden" id="hidden_1" value="INV0001">
                    <td>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#addrow").on("click", function () {
                ...
                cols += '<td><select class="inventaris select2 col-sm-12" id="inventaris_' + count + '" onchange="cbganti(value,id);" name="inventaris[]">@foreach($inventaris as $item)<option value="{{ $item->kode_inventaris }}"{{ $item->jumlah <= 0 ? ' disabled' : '' }}>{{ $item->kode_inventaris }} - {{ $item->nama_inventaris }} (jumlah: {{ $item->jumlah }})</option>@endforeach</select></td>';
                ...
                cols += '<input type="hidden" id="hidden_' + count + '" value="INV0001">';
            ...
            $.post('{{route("Inventaris.cekjumlah")}}',
                ...
                function (data) {
                    if (data.status == "lebih") {
                        $("#msgjumlah_" + $x[1]).html("<p style='color:red;'>* Jumlah input melebihi jumlah tersedia<p>");
                        $("#btnsimpan").hide();
                    }
                    else {
                        $("#msgjumlah_" + $x[1]).html("");
