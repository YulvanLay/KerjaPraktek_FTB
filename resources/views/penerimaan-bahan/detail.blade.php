...
@section('content')
    <div class="row">
        <div class="col-sm-6">
            <a class="btn btn-link" href="{{ url('terima-bahan') }}">
                < Kembali</a>
        </div>
        <div class="col-sm-6 text-right">
            @if(auth()->user()->laboran)
                <a class="btn btn-primary"
                    href="{{ action('DetailPenerimaanBahanController@tambahDetail', $penerimaan->no_PO) }}">Tambah Bahan</a>
            @endif
        </div>
    </div><br>
    <div class="row">
        ...
        <tbody>
            @foreach($penerimaan->details as $detail)
                <tr>
                    ...
                    <td>
                        {{ $detail->jumlah }}
                        {{ $detail->bahan ? $detail->bahan->satuan : '' }}
                    </td>
                    @if(auth()->user()->laboran)
                        <td>
                            <a class="btn btn-link" href="#" title="Ubah" aria-label="Ubah" data-toggle="modal"
                                data-target="#modalEdit" onclick="showModalEdit('{{ $detail->id }}');"><i
                                    class="fas fa-edit"></i></a>
                            <a class="btn btn-link" href="#" data-toggle="modal" data-target="#modalHapus" title="Hapus"
                                aria-label="Hapus"
                                onclick="updateModal('{{ $detail->bahan ? $detail->bahan->nama_bahan : $detail->kode_bahan }}'); hapus({{ $detail->id }});"><i
                ...
                <div class="modal-body">
                    <form id="formEdit" action="" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input id="jumlah" class="text-right form-control" type="number" name="jumlah" min="1"
                                step=".01" value=1></td>
