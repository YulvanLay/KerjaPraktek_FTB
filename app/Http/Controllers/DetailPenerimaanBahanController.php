...
class DetailPenerimaanBahanController extends Controller
{
    ...
    public function show($noPO)
    {
        ...
        $penerimaan = PenerimaanBahan::where('no_PO', $noPO)->first();
    ...
    public function store(Request $request)
    {
        ...
        return redirect('/terima-bahan-detail/' . $noPO)
            ->with('status', 'Berhasil menambah bahan pada No PO <strong>' . $noPO . '</strong>.')
            ->with('kode', 1);
    }
    ...
    /**
     * Show the form for editing the specified resource (stok bahan di penerimaan).
     */
    public function edit($id)
    {
        ...
        if (!$detail)
            return redirect('/terima-bahan')
                ->with('status', 'Detail penerimaan bahan tidak ditemukan.')
                ->with('kode', 0);

        $bahans = BahanLab::get();

        return view('penerimaan-bahan.detail-edit', compact('detail', 'bahans'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $hargaBaru  = $request->get('harga');

        $detail = DetailPenerimaanBahan::find($id);

        if (!$detail)
            return redirect('/terima-bahan')
                ->with('status', 'Detail tidak ditemukan.')
                ->with('kode', 0);

        if ($jumlahBaru <= 0 || $jumlahBaru == '')
            return redirect('/terima-bahan-detail/' . $detail->no_PO)
                ->with('status', 'Jumlah bahan tidak dapat kurang dari atau sama dengan 0.')
                ->with('kode', 0);
        ...
        // Kembalikan stok lama, lalu tambahkan stok baru
        $bahan->stok -= $jumlahLama;
        $bahan->stok += $jumlahBaru;

        // Update harga jika dikirim
        if (!is_null($hargaBaru) && $hargaBaru != '') {
            $bahan->harga_bahan = $hargaBaru;
        }

        $bahan->save();

        return redirect('/terima-bahan-detail/' . $detail->no_PO)
            ->with('status', 'Berhasil memperbarui stok bahan <strong>' . $bahan->nama_bahan . '</strong>.')
            ->with('kode', 1);
    }
    ...
    public function destroy($id)
    {
        ...
        return redirect('/terima-bahan-detail/' . $noPO)
            ->with('status', 'Berhasil menghapus bahan <strong>' . $bahan->nama_bahan . '</strong>.')
            ->with('kode', 1);
    }
